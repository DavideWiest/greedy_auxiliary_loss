...
def ag_news_config(beta: float, seed: int, selection: dict) -> RunConfig:
    return RunConfig(
        run_name=f"stage3_ag_news_beta{beta:.1f}_seed{seed}",
        seed=seed,
        ...
        optimizer=OptimizerConfig(lr=3e-4, weight_decay=1e-4, max_epochs=3),
...
def main() -> None:
    ...
    for beta in [0.0, 0.1, 0.2]:
        for seed in [0, 1]:
            results.append(run_experiment(ag_news_config(beta=beta, seed=seed, selection=selection)))
    ...
    write_json("results/stage3_ag_news_summary.json", summary)
